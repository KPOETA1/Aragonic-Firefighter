...
def apply_action_node(nodo, action):
    ...
    if new_world[posicion[0], posicion[1]] == 2 and nodo.agua > 0:
        ...
        return Nodo(new_world, nodo, action, nodo.cubo, nodo.agua - 1, posicion, fire=nodo.fire - 1, costo=nodo.costo + 1 + nodo.agua)
    ...
    #Verifica si la posición a la que se moverá el bombero es el hidrante
        ...
        return Nodo(new_world, nodo, action, nodo.cubo, nodo.agua + nodo.cubo, posicion, fire=nodo.fire, costo=nodo.costo + 1)
    #En caso de que no sea ninguno de los anteriores, solo se mueve
    else:
        return Nodo(new_world, nodo, action, nodo.cubo, nodo.agua, posicion, fire=nodo.fire, costo=nodo.costo + 1 + nodo.agua)
...
if __name__ == "__main__":
    nodo, path, maps, acciones = solve_costo_uniforme(world)
    print(nodo.position)
    print(nodo.fire)
    print(path)
    print(maps[-1])
